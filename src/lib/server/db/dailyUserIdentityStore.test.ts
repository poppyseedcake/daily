...
describe('SQLite Daily User identity store', () => {
  ...
  test('does not update or reschedule a deleting User during a repeated sign-in callback', async () => {
    ...
    ).run('user-1', 'google-user-1', 'original@example.com');
    const store = createDailyUserIdentityStore(drizzle(sqlite, { schema }));

    await store.upsertGoogleUser(
      { id: 'user-1', googleSubject: 'google-user-1', email: 'updated@example.com' },
      '2026-06-24T07:00:00Z'
    );

    expect(sqlite.prepare(
      'select email, lifecycle_state, next_summary_at from users where id = ?'
    ).get('user-1')).toEqual({
