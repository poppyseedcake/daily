...
Milestone 7 must turn the existing generation and test-delivery capabilities into an idempotent scheduled workflow. It must preserve section-level degradation, deliver a fixed four-section summary even when sections have no live content, keep delivery history useful without retaining private email content, and isolate failures while remaining simple enough to run once per minute from cron or a systemd timer.

## Solution

Add a scheduled Daily Summary worker to the existing application. Store each User's next scheduled summary instant as `next_summary_at` in UTC when Summary Delivery is enabled, calculate it from Summary Time and User Time Zone with Temporal, and invoke the worker every minute through a non-interactive application command.
...
The worker generates sections through the same Daily Summary generation path used by preview and test delivery. Each due occurrence contains the four fixed Summary Sections. A paused, unconfigured, empty, or unavailable section remains in the message as its status, and a provider failure becomes an Unavailable Section without blocking the other sections. Summary Delivery is the only send-eligibility control. Retryable email-provider failures receive at most three total attempts within 15 minutes of Summary Time; permanent failures and stale occurrences are not retried.
...
9. As a signed-in User, I want pausing every Summary Section to keep my pending schedule, so that Summary Delivery remains the only scheduling control.
10. As a signed-in User, I want changing a Summary Section pause to leave my pending occurrence unchanged, so that section preferences do not create a new delivery time.
...
16. As a signed-in User, I want scheduled delivery to use the same Daily Summary Appearance as preview and test delivery, so that automatic email does not look different.
...
20. As a signed-in User, I want scheduled delivery to fetch Commute Estimates live only when an active Commute Section has a route for the day, so that traffic information is current without wasting Maps usage.
...
22. As a signed-in User, I want paused Summary Sections to skip their provider calls, so that paused content cannot fail or consume provider usage.
23. As a signed-in User, I want one failed active section to appear as an Unavailable Section, so that I can still receive the other sections.
...
28. As a signed-in User, I want Daily to send all four Summary Sections when Summary Delivery is enabled, so that I receive a predictable message even when sections have no live content.
29. As a signed-in User, I want empty, paused, unconfigured, and unavailable sections to show their status, so that the message explains why live content is not present.
30. As a signed-in User, I want every scheduled provider submission to create one Delivery Record, so that delivery history represents all scheduled attempts.
31. As a signed-in User, I want a completed scheduled occurrence to advance to the next local-day occurrence, so that one quiet day does not disable future summaries.
...
- A User is schedule-eligible only when Summary Delivery is enabled. Summary Section pause settings do not affect `next_summary_at`.
...
- Recalculate `next_summary_at` after a scheduled occurrence is claimed or completed and whenever Summary Time, User Time Zone, or Summary Delivery changes. Summary Section pause changes do not recalculate it.
...
- Generate the occurrence before creating a Delivery Record. Duplicate workers may perform harmless duplicate generation, but only the invocation that atomically inserts the unique scheduled Delivery Record may submit email.
...
- Keep Summary Section pause settings authoritative and skip provider calls for paused sections.
...
- Submit every generated scheduled Daily Summary when Summary Delivery is enabled. The message contains all four fixed sections, including paused, unconfigured, empty, and unavailable states.
- Create one scheduled Delivery Record for every claimed provider submission. No content check may prevent the provider call or the Delivery Record.
...
- Test recalculation after Summary Time, User Time Zone, and Summary Delivery changes.
- Test that disabling Summary Delivery clears scheduling and that re-enabling it schedules the next future occurrence even when every Summary Section is paused.
- Test that pause-only changes keep the same scheduled occurrence identity.
...
- Test that scheduled generation reuses current Summary Configuration, Daily Summary Appearance, fixed section order, Todo state, Weather, Commute, Calendar, and renderer behavior.
- Test that paused sections perform no provider calls.
- Test Weather, Commute, and Calendar failures separately and verify each becomes an Unavailable Section without failing unrelated sections.
- Test that a scheduled Daily Summary is delivered when every section is paused, empty, unconfigured, or unavailable.
- Test that every scheduled provider submission creates one Delivery Record and advances `next_summary_at`.
...
- Each scheduled occurrence has a Delivery Record because every due occurrence makes one provider submission when Summary Delivery is enabled. Unavailable, empty, unconfigured, and paused states remain useful status information in the fixed message.
