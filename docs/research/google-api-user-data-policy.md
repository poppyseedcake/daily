...
The Calendar `events.list` endpoint accepts both `calendar.readonly` and `calendar.events.readonly`, among other scopes. The endpoint documentation therefore supports checking the actual endpoints before selecting a scope. If Daily only lists events, `calendar.events.readonly` appears narrower than `calendar.readonly` from the scope descriptions. This is an implementation inference, not a final scope decision. [Calendar `events.list` authorization](https://developers.google.com/calendar/api/v3/reference/events/list)
